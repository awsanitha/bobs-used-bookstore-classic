    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        async Task<Order> IOrderRepository.GetAsync(int id)
        {
            return await dbContext.Order
                ...
                .SingleOrDefaultAsync(x => x.Id == id) ?? throw new InvalidOperationException($"Order {id} not found");
        }

        async Task<Order> IOrderRepository.GetAsync(int id, string sub)
        {
            return await dbContext.Order.SingleOrDefaultAsync(x => x.Id == id && x.Customer.Sub == sub) ?? throw new InvalidOperationException($"Order {id} not found");
        }
